...
// guard (intentional duplication; the BACKLOG 'one per-tenant egress policy'
// item is the real de-dup). A self-host api talks to a LAN CUPS host / edge
// bridge, so on self-host we allow private ranges and block only loopback /
 ...
 *  it resolves DNS. */
export async function assertSafePrinterUrl(raw: string): Promise<void> {
  let u: URL;
  try {
    u = new URL(raw);
  } catch {
