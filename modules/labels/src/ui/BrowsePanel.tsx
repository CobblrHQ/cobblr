...
// Labels page. Tabs == the entity kinds that support labels (resolved
// server-side from the labels:print action's appliesTo — no hardcoded
// list of "parts, machines, …"); each tab lists that kind's rows with a
// one-tap Add. So a workshop can come HERE and build a label run, not
// only push into the queue from other modules.
...
export function BrowsePanel() {
  ...
  const kindsQ = useQuery({
    queryKey: ["labels-browse-kinds", orgSlug],
    queryFn: () => api.listLabelableKinds(),
    enabled: !!orgSlug,
  });
  const kinds = kindsQ.data?.kinds ?? [];

  const [active, setActive] = useState<string | null>(null);
  // Default to the first kind once they load (no useEffect — derive it).
  const activeKind = active ?? kinds[0]?.kind ?? null;

  const [q, setQ] = useState("");

  const itemsQ = useQuery({
    queryKey: ["labels-browse-items", orgSlug, activeKind, q],
    queryFn: () => api.listLabelableItems(activeKind!, { q: q.trim() || undefined, limit: 120 }),
    enabled: !!orgSlug && !!activeKind,
  });
  ...
  if (kinds.length === 0) {
    return (
      <div className="border-2 border-dashed border-line dark:border-slate-700 rounded-xl p-8 text-center text-faint dark:text-slate-500 text-sm">
        Nothing in this workspace supports labels yet. Enable a module like Inventory,
        Machines, or Assets — its items become labelable automatically.
      </div>
    );
  }
          ...
          return (
            ...
            >
              ...
              {typeof k.count === "number" && (
                <span className="ml-1.5 font-mono text-[10px] text-faint dark:text-slate-500">{k.count}</span>
              )}
            </button>
          );
        })}
