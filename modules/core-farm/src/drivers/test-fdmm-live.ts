...
// Read-only: testConnection + listPrinters + resolve. No print. Creds via
// env only (no secrets in the repo):
//   FDMM_URL=http://host:4000 FDMM_USER=u FDMM_PASS=p \
//     npx tsx modules/core-farm/src/drivers/test-fdmm-live.ts
...
console.log(`✓ listPrinters: ${printers.length} printers →`, JSON.stringify(printers.map((p) => ({ id: p.id, name: p.name, enabled: p.enabled }))));
