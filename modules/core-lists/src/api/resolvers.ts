// Entity-kind resolvers — let other modules + core-views read core-lists
...
export function registerListResolvers(): void {
  ...
  platform().entities.registerResolver("core-lists:list", async (orgId, id) => {
    const db = (await platform().tenants.getDb(orgId)) as Kysely<CoreListsDB>;
    const row = await db.selectFrom("core_lists_lists").selectAll().where("id", "=", id).executeTakeFirst();
    return row ? listToResolved(row) : null;
  });

  platform().entities.registerListResolver("core-lists:list", async (orgId, query) => {
    const db = (await platform().tenants.getDb(orgId)) as Kysely<CoreListsDB>;
    const rows = await db
  ...
  platform().entities.registerResolver("core-lists:item", async (orgId, id) => {
    const db = (await platform().tenants.getDb(orgId)) as Kysely<CoreListsDB>;
    const row = await db.selectFrom("core_lists_items").selectAll().where("id", "=", id).executeTakeFirst();
    return row ? itemToResolved(row) : null;
  });

  platform().entities.registerListResolver("core-lists:item", async (orgId, query) => {
    const db = (await platform().tenants.getDb(orgId)) as Kysely<CoreListsDB>;
    let q = db.selectFrom("core_lists_items").selectAll();
