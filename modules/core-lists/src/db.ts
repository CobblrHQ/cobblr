// Kysely table types for core-lists. Column names match the migration
...
export interface CoreListsDB {
  core_lists_lists: ListsTable;
  core_lists_items: ItemsTable;
  core_lists_expiry_notifications: ExpiryNotificationsTable;
}
...
export function tenantDb(req: Request): Kysely<CoreListsDB> {
  const t = (req as unknown as RequestWithTenant).tenant;
  if (!t) throw new Error("core-lists route called without tenant context");
  return t.db as Kysely<CoreListsDB>;
}
