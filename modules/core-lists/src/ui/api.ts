// Thin fetch wrapper for the core-lists REST surface. Auth via getToken().
// Routes are mounted at /api/v1/orgs/:slug/modules/core-lists/...
...
export class ListsApi {
  ...
  private async req<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    const res = await fetch(`/api/v1/orgs/${this.slug}/modules/core-lists${path}`, {
      method,
      headers,
