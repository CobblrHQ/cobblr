// core-lists UI — the host mounts <ListsUI /> at /lists. A grid of lists;
...
export function ListsUI({ orgSlug, getToken }: Props) {
  ...
  const createList = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreating(false);
      setNewTitle("");
      void qc.invalidateQueries({ queryKey: ["core-lists", orgSlug] });
    },
...
function ListDetailModal({ listId, api, onClose }: { listId: string; api: ListsApi; onClose: () => void }) {
  ...
  const detail = useQuery({ queryKey: ["core-lists-detail", listId], queryFn: () => api.getList(listId) });

  const invalidate = () => {
    void qc.invalidateQueries({ queryKey: ["core-lists-detail", listId] });
    void qc.invalidateQueries({ queryKey: ["core-lists"] }); // refresh counts on the grid
  };
