...
export interface CompatPresetOpts {
  ...
  /** The short list of models a person may PICK for a job, when the preset
   *  knows them. The compat family declares "default" because a bare gateway
   *  serves whatever is loaded; a preset that fronts a known provider can say
   *  "these two, and here is what each costs". Absent: the default only. */
  modelsFor?: Partial<Record<AiCapability, string[]>>;
  /** Labels and one-line notes for those ids, for the picker. */
...
export function buildCompatProvider(opts: CompatPresetOpts): AiProviderDef {
  ...
  return {
    ...
    setup: opts.setup,
    rank: opts.rank,
    capabilities: opts.modelsFor
      ? Object.fromEntries(
          Object.entries(SUPPORTED).map(([cap, def]) => {
            const menu = opts.modelsFor?.[cap as AiCapability];
            return [
              cap,
              menu?.length
                ? { models: menu, defaultModel: opts.modelForCapability?.[cap as AiCapability] ?? opts.defaultModel ?? menu[0] }
                : def,
            ];
          }),
        )
      : SUPPORTED,
    ...(opts.modelNotes ? { modelNotes: opts.modelNotes } : {}),
