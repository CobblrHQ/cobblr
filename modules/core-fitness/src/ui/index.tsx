// core-fitness UI — host mounts <FitnessUI /> at /tracking. A grid of metrics
...
import { FitnessApi, type MetricSummary, type Measurement, type GoalDirection, FitnessApiError } from "./api.js";
...
export function FitnessUI({ orgSlug, getToken }: Props) {
  usePageTitle("Tracking");
  const api = new FitnessApi(orgSlug, getToken);
  ...
  const metrics = useQuery({ queryKey: ["core-fitness", orgSlug], queryFn: () => api.listMetrics() });

  const del = useMutation({
    mutationFn: (id: string) => api.deleteMetric(id),
    onSuccess: () => { toast.success("Metric deleted"); void qc.invalidateQueries({ queryKey: ["core-fitness", orgSlug] }); },
    onError: (e) => toast.error(e instanceof FitnessApiError ? e.message : String(e)),
  });
...
function MetricCard({ metric, api, orgSlug, onOpen, onDelete }: { metric: MetricSummary; api: FitnessApi; orgSlug: string; onOpen: () => void; onDelete: () => void }) {
  const qc = useQueryClient();
  const toast = useToast();
  ...
  const log = useMutation({
    ...
    onSuccess: () => {
      setVal("");
      void qc.invalidateQueries({ queryKey: ["core-fitness", orgSlug] });
      void qc.invalidateQueries({ queryKey: ["core-fitness-detail", metric.id] });
    },
    onError: (e) => toast.error(e instanceof FitnessApiError ? e.message : String(e)),
  });
  return (
...
function CreateMetricModal({ api, orgSlug, onClose }: { api: FitnessApi; orgSlug: string; onClose: () => void }) {
  const qc = useQueryClient();
  const toast = useToast();
  ...
  const create = useMutation({
    ...
    onSuccess: () => { toast.success("Metric created"); void qc.invalidateQueries({ queryKey: ["core-fitness", orgSlug] }); onClose(); },
    onError: (e) => toast.error(e instanceof FitnessApiError ? e.message : String(e)),
  });
...
function MetricDetailModal({ metricId, api, orgSlug, onClose }: { metricId: string; api: FitnessApi; orgSlug: string; onClose: () => void }) {
  const qc = useQueryClient();
  const toast = useToast();
  const [val, setVal] = useState("");
  const detail = useQuery({ queryKey: ["core-fitness-detail", metricId], queryFn: () => api.getMetric(metricId) });

  const invalidate = () => {
    void qc.invalidateQueries({ queryKey: ["core-fitness-detail", metricId] });
    void qc.invalidateQueries({ queryKey: ["core-fitness", orgSlug] });
  };
  const log = useMutation({
