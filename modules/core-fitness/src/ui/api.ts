// Thin fetch wrapper for the core-fitness REST surface. Auth via getToken().
// Routes mounted at /api/v1/orgs/:slug/modules/core-fitness/...
...
export class FitnessApi {
  ...
  private async req<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    const res = await fetch(`/api/v1/orgs/${this.slug}/modules/core-fitness${path}`, {
      method,
      headers,
    ...
    try {
      ...
      throw new FitnessApiError(res.status, "non_json", `Non-JSON response (${res.status})`);
    }
    if (!res.ok) {
      const e = (parsed as { error?: { code?: string; message?: string } }).error;
      throw new FitnessApiError(res.status, e?.code ?? "error", e?.message ?? `Request failed (${res.status})`);
    }
    return parsed as T;
  }
