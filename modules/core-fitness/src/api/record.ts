...
// (core-fitness:log-measurement) funnel through here so the goal-reached
// check + notification fire identically no matter how a value arrives.

import { type Kysely } from "kysely";
import { platform } from "@cobblr/platform-contract";
import type { CoreFitnessDB, MeasurementsTable } from "../db.js";
...
export async function logMeasurement(opts: {
  ...
  void platform().events.emit("core-fitness.measurement.logged", { orgId, metricId: metric.id, value });

  const goal = num(metric.goal_value);
  if (goal != null && goalReached(value, goal, metric.goal_direction)) {
    void platform().events.emit("core-fitness.goal.reached", { orgId, metricId: metric.id, value, goal });
    try {
      ...
      for (const userId of memberIds) {
        await platform().notifications.dispatch({
          orgId,
          userId,
          eventType: "core-fitness.goal-reached",
          message: `🎯 Goal reached — ${metric.name} hit ${goal}${unit}`,
          module: "core-fitness",
          entityType: "core-fitness:metric",
          entityId: metric.id,
          payload: { value, goal },
        });
      }
    } catch (err) {
      console.error("[core-fitness] goal-reached notify failed:", (err as Error).message);
    }
  }
  return row;
}
