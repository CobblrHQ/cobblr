// core-fitness action handlers. `core-fitness:log-measurement` is the wire
...
// measurement` without core-fitness ever knowing purchases exists.
//
// The metric is resolved by id or by name (created on miss, like
// core-lists:add-item creates a list on miss). The value is resolved, in order,
...
export function registerFitnessActionHandlers(): void {
  if (registered) return;
  registered = true;

  platform().actions.registerHandler("core-fitness.log-measurement", async (ctx) => {
    const args = (ctx.args as LogArgs | null) ?? {};
    const db = (await platform().tenants.getDb(ctx.orgId)) as Kysely<CoreFitnessDB>;
    ...
    if (!metric && args.metricName) {
      ...
      if (!metric) {
        ...
        void platform().events.emit("core-fitness.metric.created", { orgId: ctx.orgId, metricId: metric.id });
      }
    }
