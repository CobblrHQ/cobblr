...
export async function enrichPhotoItem(ctx: PhotoEnrichContext): Promise<void> {
  ...
  try {
    const r = await platform().ai.invoke({
      ...
      input: { image_b64: imageB64, image_media_type: file.mimeType },
      source: { kind: "core-scan:photo", id: ctx.itemId },
    });
    ...
    // No vision provider, or the model/parse failed — leave a review draft.
    await patchNote(
      ctx,
      "Photo couldn't be auto-identified (no vision provider configured, or the model errored). Fill in manually.",
    );
    return;
  }
  ...
  if (!name) {
    await patchNote(ctx, "Vision couldn't make out a single item in the photo. Fill in manually.");
    return;
  }
  ...
  await ctx.db
    ...
    .set({
      suggested_name: name,
      suggested_manufacturer: typeof parsed?.brand === "string" ? parsed.brand.trim() || null : null,
