        ...
        <div className="text-xs font-mono space-y-1">
          ...
          {callsQ.data?.items.map((c) => (
            <div key={c.id} className="flex gap-3 items-center">
              ...
              <span className="text-slate-500">{c.direction}</span>
              <span className="font-medium">{c.connector_id}</span>
              <span className="text-slate-500">{c.action_or_event}</span>
    ...
    <div className="border rounded p-3 dark:border-slate-700">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs font-mono mt-1 text-slate-500 break-all">{fullUrl}</div>
        </div>
        <div className="flex items-center gap-1">
          <button
            type="button"
  ...
  return (
    <Modal open onClose={onClose} title={`Send via ${connector.label}`} size="md">
      <div className="space-y-3">
        ...
        {result !== null && (
          <pre className="text-xs bg-slate-50 dark:bg-slate-900 border dark:border-slate-700 rounded p-2 overflow-auto max-h-40">
