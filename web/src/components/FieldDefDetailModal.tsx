    ...
    >
      <div className="space-y-4">
        <dl className="grid grid-cols-2 gap-2 text-xs">
          ...
          <Row label="Origin">
            {fieldDef.bundle_id ? (
              <span className="text-cobble-600 dark:text-cobble-300">bundled</span>
            ) : (
        ...
        <div className="p-3 rounded-md bg-mortar-50 dark:bg-slate-800/70 text-xs text-slate-600 dark:text-mortar-200">
          <div className="font-mono uppercase tracking-widest text-[10px] text-slate-400 mb-1">
            Where it appears
          </div>
          Renders on every <code className="font-mono text-cobble-600">{fieldDef.entity_kind}</code>{" "}
          detail page under a <em>custom fields</em> section, and the value is
          accessible in templates as <code className="font-mono text-cobble-600">{`{{${fieldDef.name}}}`}</code>.
        </div>
