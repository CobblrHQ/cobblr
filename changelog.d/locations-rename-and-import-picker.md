---
type: feature
date: 2026-06-22
---
**Rename the Locations nav entry to your word.** The top-level **Locations** entry now honors the org-wide display-name override (Configuration → Presentation) — call it "Rooms", "Areas", whatever fits your workspace, consistent on every device.

**Locations CSV import: pick the match column from a dropdown.** The "match existing on" control now lists the actual columns from your CSV's header row instead of a free-text box — no typing/typo'ing the id column.
