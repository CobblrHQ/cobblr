// Shared by the api (server-side auto-apply) and the web (the dashboard's
// report and the explicit prompt), so the two can never disagree on which
// updates apply on their own. Lived in web/src/lib until the apply moved
// server-side (2026-09-09); the web file re-exports this.
